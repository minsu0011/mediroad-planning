...
from .runner import run_top3_semantic_aggregate


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="MEDIROAD Stage 4.2F Top3 required-scenario semantic aggregate"
    )
    ...
    parser.add_argument(
        "--stage42f-config",
        type=Path,
        default=Path("configs/model_v1/stage4_2f_top3_aggregate.yaml"),
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_top3_semantic_aggregate(
        ...
        stage42f_config_path=args.stage42f_config,
    )
