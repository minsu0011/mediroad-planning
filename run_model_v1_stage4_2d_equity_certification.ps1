...
param(
    ...
    [string]$Config = "configs/model_v1/stage4_2d_equity_certification.yaml",
...
function Quote-BashLiteral {
    ...
    if ($Value.Contains("'")) { throw "Unsupported single quote in WSL argument: $Value" }
    return "'$Value'"
}
...
# WSL2 is the validated CUDA/HiGHS environment for this package and is the
# default. -NativeWindows is an explicit escape hatch for an equivalent env.
...
if ($EffectiveUseWsl) {
    ...
    $WslArgs = @(
        ...
        "$WslPrefix/bin/python", "-m", "mediroad.stage4_2d_equity_certification",
...
& conda run -n $CondaEnv --no-capture-output python -m mediroad.stage4_2d_equity_certification `
    --project-root "$PWD" `
    --stage42-config configs/model_v1/stage4_2.yaml `
    --stage42c-config configs/model_v1/stage4_2_certification.yaml `
    --stage42d-config $Config
exit $LASTEXITCODE
